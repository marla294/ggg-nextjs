...
export default async ({quantity, ingredientId, recipeId}: {quantity: any, ingredientId: any, recipeId: any}) => {
  try {
    ...
    const recipeItem = await RecipeItem.create([{quantity: (quantity || 1) * 10, user: new ObjectId(user._id), ingredient: new ObjectId(ingredientId), recipe: new ObjectId(recipeId)}]);
